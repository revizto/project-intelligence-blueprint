...
**Status: v1.0.0-rc.6 — private release candidate (build `2026-07-20.1`).** Public repo (required for the desktop install path), production deep-links (`ws.revizto.com`), read-only by default, ships with an empty connector set and a synthetic demo snapshot. Not for public distribution until sign-off.
...
| **"The Blueprint's tools aren't authorised for this artifact yet"** | Gate 2: the artifact's `mcp_tools` allowlist is empty. `create_artifact` only binds a tool if it was **called in the same session**, so declaring the read tools without calling them leaves the allowlist empty | Use **rc.6+** and re-run the install skill: it now calls each of the nine read tools once, then declares them. Delete the empty artifact and re-run. (Connecting the connector alone never authorises the artifact's tools — that's a separate step done at `create_artifact` time.) |
